...
MODEL_OLLAMA = "openai/qwen3:latest"
...
#     model="ollama/llama3:8b",  # or "mistral", "phi", etc.
...
# Define the weather agent using a small local Ollama model
weather_agent = Agent(
    model=LiteLlm(model=MODEL_OLLAMA,   model_type="openai",   api_key="ollama-secret-key" ,api_base='http://localhost:11434/v1',),  # or just "mistral" if that's your model name
    name="weather_agent_v1",
    description="Provides weather information for specific cities.",
    instruction="""
      You are a helpful weather assistant. When the user asks for the weather in a specific city,
      use the 'get_weather' tool to find the information. If the tool returns an error, inform the
      user politely. If the tool is successful, present the weather report clearly.
    """,
    tools=[
        get_weather,
    ],
...
print(f"Agent '{weather_agent.name}' created using model 'Ollama'.")
...
if greeting_agent and farewell_agent and 'get_weather' in globals():
    ...
    print(f"✅ Root Agent '{weather_agent_team.name}' created using root model with sub-agents: {[sa.name for sa in weather_agent_team.sub_agents]}")
